...
class WebInterface(Node):
    ...
    def __init__(self):
        ...
        # Visual servo publisher — triggers pick-and-place in visual_servo_node.py
        self.pick_pub = self.create_publisher(Int32, "/visual_servo/pick_request", 10)

        # Inventory — only initialise tables, do NOT auto-reset data
        if _INV_AVAILABLE:
            init_db()
            count = stock_count()
            self.get_logger().info(
                f"DB initialised — {count} item(s) in stock. "
                "Run 'ros2 run dexter_inventory seed_data --clear' to populate."
            )
            self.forecaster = DemandForecaster()
        else:
            self.get_logger().warn(
                "dexter_inventory not available — install it and rebuild."
            )
...
def main():
    ...
    ros_node.get_logger().info("  Dispatch: visual_servo (/visual_servo/pick_request)")
